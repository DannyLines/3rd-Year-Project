...
def preprocess(df):
    new_df = df.copy()
    new_df['HT_acc'] = (new_df['HST'] / new_df['HS'])
    new_df['HT_acc'].replace([np.inf, -np.inf], np.nan)
    new_df['HT_acc'].fillna(0)
    ...
    new_df['AT_acc'] = (new_df['AST'] / new_df['AS'])
    new_df['AT_acc'] = new_df['AT_acc'].fillna(0)
    print(new_df['AT_acc'])
    return new_df
...
def main():
    ...
    for row in fixture_data['FTR']:
        if row == 0:
            ...
            colors.append("black")
                ...
                colors.append("green")
    # plt.figure()
    # plt.scatter(fixture_data['HTGD'], fixture_data['HTGD'], color=colors, alpha=0.01, s=50)
    # plt.xlabel("Half time goal difference")
    # plt.ylabel("Half time goal difference (relative to home team)")
    # plt.legend(['Green - Home Win \nRed - Away Win \nBlack - Draw'])
    #plt.scatter(away_goals_figures, results)
    plt.show()
    ...
    #col_names = poly.get_feature_names(data.columns)
    #print(col_names)
