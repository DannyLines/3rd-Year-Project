...
def preprocess(df):
    new_df = df.copy()
    new_df['MSH_home_acc'] = (new_df['MSH_home_target'] / new_df['MSH_home_attempts'])
    new_df['MSH_home_acc'] = new_df['MSH_home_acc'].replace([np.inf, -np.inf], np.nan)
    new_df['MSH_home_acc'] = new_df['MSH_home_acc'].fillna(0)
    ...
    new_df['MSH_away_acc'] = (new_df['MSH_away_target'] / new_df['MSH_away_attempts'])
    new_df['MSH_away_acc'] = new_df['MSH_away_acc'].replace([np.inf, -np.inf], np.nan)
    new_df['MSH_away_acc'] = new_df['MSH_away_acc'] = new_df['MSH_away_acc'].fillna(0)
...
def main():
    ...
    data = data[['MFH_home_sentiment', 'MFH_away_sentiment']]
    print(target_data)
    #new_data_df = preprocess(data)
    data['FTR'] = target_data
    ...
    best_model_params = getBestHyperParamters(data, splits, depth, estimators, tLabel)
    best_model = RandomForestClassifier(n_estimators=500, max_depth=best_model_params[1], random_state=0)

    #print(new_data_df.columns.tolist())

    data = data.drop('FTR', axis=1)
    best_model.fit(data, target_data)

    #print(best_model.columns.tolist())
    #test = pd.DataFrame([[19, 1, 2, 0.4, 2.4, -2, 0, 2.25, 1.75, 0.5, 2.25, 0, 0.391667, 1, 2, 36, 64, 12, 21, 5, 7, 0.416666, 0.3333333]])

    #print(best_model.predict_proba(test))


    filename = 'MFH_PureSentiment_PredictionModel.sav'
